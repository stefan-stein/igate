...
Using the `r params$df_name` data set, we conducted a `r params$versus` good versus `r params$versus` bad `r params$type` pairwise comparison analysis on `r Sys.Date()`. The target variable was `r params$target`. The goal of the analysis was to find process parameters in the `r params$df_name` data set that significantly influence the values of `r params$target`.
...
The results and images are stored in `r params$image_directory`.
...
kable(validation_summary,
      row.names = FALSE,
      caption = 'Summary of the table above.',
      booktabs = TRUE,
      longtable = TRUE)%>%
  kable_styling(latex_options = c("hold_position", "repeat_header"))
}
